...
void UpdateListMaterial(S_BOARD *pos) {
    ...
    for (index = 0; index < BRD_SQ_NUM; ++index) {
        ...
        if (piece != OFFBOARD && piece != EMPTY) {
            ...
            if (piece == bK) pos->KingSq[BLACK] = sq;  /* was wK — fixed */
...
int ParseFen(char *fen, S_BOARD *pos) {
    ...
    UpdateListMaterial(pos);   /* was missing */
...
void ResetBoard(S_BOARD *pos) {
    ...
    for (index = 0; index < BRD_SQ_NUM; ++index) pos->pieces[index] = OFFBOARD;
    for (index = 0; index < 64; ++index)          pos->pieces[SQ120(index)] = EMPTY;

    for (index = 0; index < 2; ++index) {          /* bigPce/majPce/minPce are [2] — fixed loop */
        pos->bigPce[index]   = 0;
        pos->majPce[index]   = 0;
        pos->minPce[index]   = 0;
...
void PrintBoard(const S_BOARD *pos) {
    ...
    printf("\n");
    printf("side: %c\n", SideChar[pos->side]);   /* was "side:c%" — fixed */
    printf("enPas: %d\n", pos->enPas);
